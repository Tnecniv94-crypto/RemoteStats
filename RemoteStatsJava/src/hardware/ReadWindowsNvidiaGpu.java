...
public class ReadWindowsNvidiaGpu {
	...
	protected void readGpusTemperatureToServer(String ip, int port, String token, int sleepSec) {
		...
		while (running) {
			...
			System.out.println(client.sendMessage("update _temps; " + temps, ip, port, token));
	...
	private String getCurrentNvidiaWindowsTemperature(int gpus) {
		...
		ret += mapping[mapping.length - 1][0] + "; ";
		ret += "names: ";
		...
		ret += mapping[mapping.length - 1][1] + "; ";
		ret += "temps: ";
	...
	private String getCurrentNvidiaWindowsFanSpeed(int gpus) {
		...
		for (int i = 0; i < gpus - 1; i++) {
			...
			buf = line.split("%")[0];
			ret += buf.substring(2, buf.length()) + "%, ";
		}
	...
	/**
	 * 
	 * @return the whole nvidia-smi response for "nvidia-smi.exe --query
	 *         --display=TEMPERATURE"
	 */
	 ...
	 * 
	 * @param param=TEMP: "nvidia-smi.exe --query --display=TEMPERATURE",
	 *                    param=POWER: "nvidia-smi.exe --query --display=POWER",
	 *                    param=NAME: "nvidia-smi.exe -L"
	 */
